...
save_results = '/home/andreas/MicroHH/Results/fireles_outflow/thlpath/'

file = "thlpath.xy.nc"
with nc.Dataset(file) as ncf:
	print(ncf)
	#th = ncf.variables['thl'][:]
	s = ncf.variables['thlpath'][:]
	x = ncf.variables['x'][:]
	y = ncf.variables['y'][:]
...
for i, vel in enumerate(s):
    ...
    ax.set_title('xy cross-section of thlpath t={time:.1f}h'.format(time=time[i]/3600))
    ax.set_ylabel('y size of domain[m]')
    ax.set_xlabel('x size of domain[m]')
    
    fig.colorbar(cs)
    fig.savefig(save_results + 'thlpath_%4.4i.png'%(i), dpi = 'figure')
    plt.close(fig)	
	#plt.show()
