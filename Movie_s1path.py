...
save_results = '/home/andreas/MicroHH/Results/fireles_outflow/s1path/'

file = "s1path.xy.nc"
with nc.Dataset(file) as ncf:
	print(ncf)
	
	s = ncf.variables['s1path'][:]
	x = ncf.variables['x'][:]
	y = ncf.variables['y'][:]
...
for i, vel in enumerate(s):
    ...
    ax.set_title('xy cross-section of s1path t={time:.1f}h'.format(time=time[i]/3600))
    ax.set_ylabel('y size of domain[m]')
    ax.set_xlabel('x size of domain[m]')
    
    fig.colorbar(cs)
    fig.savefig(save_results + 's1path_%4.4i.png'%(i), dpi = 'figure')
    plt.close(fig)	
	#plt.show()
