...
save_results = '/home/andreas/MicroHH/Results/fireles_no_fire/thlpath/'

file = "thlpath.xz.nc"
with nc.Dataset(file) as nc:
    print(nc.variables)
    s = nc.variables['thlpath'][:]
    x = nc.variables['x'][:]
    y = nc.variables['y'][:]
    z = nc.variables['z'][:]
...
for i in range(len(s)): 
    ...
    fig.colorbar(cs)
    ax.set_title('xz cross-section of thl_path, t={time:.1f}h'.format(time=time[i]/3600))
    ax.set_xlabel("x domain")
    ax.set_ylabel("z domain")
    fig.savefig(save_results + 'thl_path_%4.4i.png'%(i), dpi='figure')
    plt.close(fig)
