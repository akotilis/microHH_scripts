...
save_results = '/home/andreas/MicroHH/Results/fireles_no_fire/qlpath/'
...
for i, vel in enumerate(s):
    ...
    ax.set_title('xy cross-section of qlpath t={time:.1f}h'.format(time=time[i]/3600))
    ax.set_ylabel('y size of domain[m]')
    ax.set_xlabel('x size of domain[m]')
    
    fig.colorbar(cs)
    fig.savefig(save_results + 'qlpath_%4.4i.png'%(i), dpi = 'figure')
    plt.close(fig)	
	#plt.show()
